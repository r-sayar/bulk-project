...
└── src/                     # All source code, outputs, and documentation
    ├── scripts/
    ...
    │       └── deconvolution_of_bulk_rna_seq_using_deep_learning.py  # DL deconvolution
...
```bash
pip install -r src/CDState/cdstate_requirements.txt
```
...
```bash
python src/scripts/eda/preprocessing_qc.py
```
...
python src/scripts/eda/single_cell_analysis.py

# 3. Bulk RNA-seq analysis
python src/scripts/eda/gene_expression_analysis.py

# --- Deconvolution ---
# 4. Launch Gradio deconvolution demo
python src/scripts/deconvolution/app_gradio.py
```
